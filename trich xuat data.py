...
def extractingShape(img_path, cell_size=8, block_size=2, bins=9):
    ...
    return feature_tensor.flatten()  # 3780 featurexs
def trichxuat(folders):
    result = []
    for i,folder in enumerate(folders):
        ...
        for file in files:
            ...
            color = extractingColor( file)
            shape = extractingShape(file)
            v = np.concatenate((color, shape,[i]), axis=0)
            result.append(v)
    return np.array(result)
if __name__=='__main__':


    folder=['doraemon','jerry','nobita','tom']
